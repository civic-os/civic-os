...
/**
 * DOMPurify requires a browser-grade HTML parser. happy-dom (used by Angular's
 * Vitest builder) does not implement the parsing APIs that DOMPurify relies on,
 * so sanitization tests that verify tag stripping or attribute preservation are
 * skipped. They should be covered by Playwright/browser-based tests instead.
 *
 * The iframe domain allowlisting tests (our custom hook) still work because
 * they only check whether our uponSanitizeElement hook removes disallowed iframes.
 */
const describeIfBrowser = (() => {
    try {
        // Test whether DOMPurify actually sanitizes in this environment.
        // happy-dom's DOMParser "works" but DOMPurify passes <script> through.
        const purify = DOMPurify(window);
        const result = purify.sanitize('<p>ok</p><script>x</script>');
        return !result.includes('<script') ? describe : describe.skip;
    } catch {
...
describe('markdownSanitize', () => {
    ...
    describeIfBrowser('sanitization (requires browser-grade DOM parser)', () => {
