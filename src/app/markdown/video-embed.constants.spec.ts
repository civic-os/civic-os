...
describe('video-embed.constants', () => {
    ...
    describe('isAllowedEmbedDomain', () => {
        it('should allow exact domain matches', () => {
            expect(isAllowedEmbedDomain('youtube.com')).toBe(true);
            expect(isAllowedEmbedDomain('youtu.be')).toBe(true);
            expect(isAllowedEmbedDomain('youtube-nocookie.com')).toBe(true);
        });

        it('should allow www-prefixed domains', () => {
            expect(isAllowedEmbedDomain('www.youtube.com')).toBe(true);
            expect(isAllowedEmbedDomain('www.youtube-nocookie.com')).toBe(true);
        });
        ...
        it('should reject spoofed domains (suffix attack)', () => {
            expect(isAllowedEmbedDomain('youtube.com.evil.com')).toBe(false);
            expect(isAllowedEmbedDomain('notyoutube.com')).toBe(false);
            expect(isAllowedEmbedDomain('evil-youtube.com')).toBe(false);
        });

        it('should reject unrelated domains', () => {
            expect(isAllowedEmbedDomain('vimeo.com')).toBe(false);
            expect(isAllowedEmbedDomain('example.com')).toBe(false);
            expect(isAllowedEmbedDomain('evil.com')).toBe(false);
        });
    });
