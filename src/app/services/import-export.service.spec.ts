...
import { EntityPropertyType, SchemaEntityTable, SchemaEntityProperty, ForeignKeyLookup, ValidationErrorSummary, ImportError } from '../interfaces/entity';

describe('ImportExportService', () => {
    let service: ImportExportService;
    let mockDataService: any;
    let mockSchemaService: any;
    let saveWorkbookSpy: any;
    ...
    beforeEach(() => {
        ...
        saveWorkbookSpy = vi.spyOn(service as any, 'saveWorkbook').mockImplementation(() => {
        });
    });
    ...
    describe('fetchForeignKeyLookups() - Observable Tests', () => {
        it('should return empty Map when no FK or User properties', async () => {
        ...
        it('should fetch FK lookup for ForeignKeyName property', async () => {
            ...
            mockDataService.getData.mockReturnValue(of(mockStatuses));
        ...
        it('should fetch FK lookup for User property', async () => {
            ...
            mockDataService.getData.mockReturnValue(of(mockUsers as any));
        ...
        it('should handle duplicate display names correctly', async () => {
            const fkProp = createMockProperty({
                column_name: 'status_id',
                display_name: 'Status',
                data_type: 'int4',
        ...
        it('should handle case-insensitive display name lookup', async () => {
            const fkProp = createMockProperty({
                column_name: 'status_id',
                display_name: 'Status',
                data_type: 'int4',
        ...
        it('should handle empty FK reference data', async () => {
            const fkProp = createMockProperty({
                column_name: 'status_id',
                display_name: 'Status',
                data_type: 'int4',
    ...
    describe('downloadErrorReport()', () => {
        it('should generate error report and call saveWorkbook', () => {
            ...
            const [, filename] = vi.mocked(saveWorkbookSpy).mock.lastCall;
        ...
        it('should include workbook with Error Rows and Summary sheets', () => {
            ...
            const [workbook] = vi.mocked(saveWorkbookSpy).mock.lastCall;
    ...
    describe('generateUserImportTemplate()', () => {
        it('should generate user import template with 6 column headers', () => {
            ...
            expect(saveWorkbookSpy).toHaveBeenCalledTimes(1);
            const [workbook, filename] = vi.mocked(saveWorkbookSpy).mock.lastCall;
        ...
        it('should call saveWorkbook with correct filename', () => {
            ...
            expect(saveWorkbookSpy).toHaveBeenCalledWith(expect.anything(), 'user_import_template.xlsx');
        });
    });
    ...
    describe('EntityPropertyType Worker Sync', () => {
         ...
         */
        const EXPECTED_WORKER_ENUM_VALUES: {
            [key: string]: number;
        } = {
            Unknown: 0,
            TextShort: 1,
        ...
        it('should have matching enum values', () => {
            ...
            for (const key of enumKeys) {
                ...
                expect(enumValue, `EntityPropertyType.${key} should equal ${expectedValue} (worker expects this value)`).toBe(expectedValue);
            }
        });
        ...
        it('should fail if a new type is added without updating worker', () => {
            ...
            expect(enumKeys.length, 'New EntityPropertyType detected! Update:\n' +
                '  1. import-validation.worker.ts\n' +
                '  2. EXPECTED_WORKER_ENUM_VALUES in this test').toBe(28); // Unknown(0) through Markdown(27) = 28 types
        });
    });
    ...
    describe('exportToExcel() - VIEW FK filtering', () => {
        ...
        beforeEach(() => {
            mockDataService.getDataPaginated.mockReturnValue(of({ data: [], totalCount: 1 }));
            mockDataService.getData.mockReturnValue(of([{ id: 1, project: 'Test Project', project_id: 5, display_name: 'Test', created_at: '2025-01-01', updated_at: '2025-01-01' }]));
        });
        ...
        it('should exclude hidden FK properties from VIEW export', async () => {
            ...
            const getDataCallArgs = vi.mocked(mockDataService.getData).mock.lastCall!;
            const fields: string[] = getDataCallArgs[0].fields;

            // Hidden FK column should be excluded (would generate broken PostgREST embedding)
            expect(fields.some(f => f.includes('projects!'))).toBe(false);
        ...
        it('should keep hidden FK properties for regular table export', async () => {
            ...
            expect(fields.some(f => f.includes('statuses!'))).toBe(true);
        });
        ...
        it('should keep visible FK properties on VIEW export', async () => {
            ...
            expect(fields.some(f => f.includes('categories!'))).toBe(true);
        });
        ...
        it('should exclude hidden User and Status properties from VIEW export', async () => {
            ...
            expect(fields.some(f => f.includes('civic_os_users!'))).toBe(false);
            expect(fields.some(f => f.includes('statuses!'))).toBe(false);
        ...
        it('should use display_name for row count check instead of id', async () => {
            ...
            const paginatedCallArgs = vi.mocked(mockDataService.getDataPaginated).mock.lastCall!;
            // Row count check should use display_name (always exists), not id (may not exist on VIEWs)
            expect(paginatedCallArgs[0].fields).toContain('display_name');
            expect(paginatedCallArgs[0].fields).not.toContain('id');
        });
        ...
        it('should pass isSummaryView flag on data queries for summary VIEWs', async () => {
            ...
            expect(paginatedCallArgs[0].isSummaryView).toBe(true);
            expect(getDataCallArgs[0].isSummaryView).toBe(true);
        });
        ...
        it('should not set isSummaryView for regular table exports', async () => {
            ...
            expect(paginatedCallArgs[0].isSummaryView).toBe(false);
            expect(getDataCallArgs[0].isSummaryView).toBe(false);
        });
    });
    ...
    describe('M:M Import Support', () => {
        ...
        describe('fetchForeignKeyLookups() with M:M', () => {
            it('should fetch M:M target table lookups for pure junctions', async () => {
                ...
                mockDataService.getData.mockReturnValue(of(mockCategories));
            ...
            it('should skip parent-hop M:M', async () => {
            ...
            it('should deduplicate M:M targets referencing the same table', async () => {
        ...
        describe('downloadTemplate() with M:M', () => {
            it('should include pure M:M in template', async () => {
                ...
                const workbook = vi.mocked(saveWorkbookSpy).mock.lastCall![0];
