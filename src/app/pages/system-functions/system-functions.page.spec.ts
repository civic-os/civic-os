...
describe('SystemFunctionsPage', () => {
    ...
    beforeEach(async () => {
        mockIntrospection = {
            getFunctions: vi.fn().mockName("IntrospectionService.getFunctions")
        };
        mockIntrospection.getFunctions.mockReturnValue(of([
            createMockFunction(),
    ...
    it('should toggle function expansion', () => {
        expect(component.isExpanded('test_function')).toBe(false);
        component.toggleFunction('test_function');
        expect(component.isExpanded('test_function')).toBe(true);
        component.toggleFunction('test_function');
        expect(component.isExpanded('test_function')).toBe(false);
    });
