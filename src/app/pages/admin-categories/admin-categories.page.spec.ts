...
describe('AdminCategoriesPage', () => {
    ...
    beforeEach(async () => {
        mockCategoryAdmin = {
            getCategoryEntityTypes: vi.fn().mockName("CategoryAdminService.getCategoryEntityTypes"),
            getCategoriesForEntity: vi.fn().mockName("CategoryAdminService.getCategoriesForEntity"),
            upsertCategoryGroup: vi.fn().mockName("CategoryAdminService.upsertCategoryGroup"),
            deleteCategoryGroup: vi.fn().mockName("CategoryAdminService.deleteCategoryGroup"),
            upsertCategory: vi.fn().mockName("CategoryAdminService.upsertCategory"),
            deleteCategory: vi.fn().mockName("CategoryAdminService.deleteCategory")
        };
        mockSchema = {
            invalidateCategoryCache: vi.fn().mockName("SchemaService.invalidateCategoryCache")
        };

        mockCategoryAdmin.getCategoryEntityTypes.mockReturnValue(of(mockGroups));
        mockCategoryAdmin.getCategoriesForEntity.mockReturnValue(of(mockCategories));
    ...
    describe('Group CRUD', () => {
        ...
        it('should submit group successfully', () => {
            mockCategoryAdmin.upsertCategoryGroup.mockReturnValue(of({ success: true }));
            component.openCreateGroupModal();
    ...
    describe('Category CRUD', () => {
        ...
        it('should submit category successfully', () => {
            mockCategoryAdmin.upsertCategory.mockReturnValue(of({ success: true, body: { id: 3 } }));
            component.openCreateCategoryModal();
            component.updateCategoryFormField('displayName', 'Industrial');
            component.submitCategory();
            expect(mockCategoryAdmin.upsertCategory).toHaveBeenCalledWith('building_type', 'Industrial', undefined, '#3B82F6', 2, undefined);
        ...
        it('should handle delete category with error response', () => {
            mockCategoryAdmin.deleteCategory.mockReturnValue(of({
                success: false,
    ...
    describe('Delete Group', () => {
        it('should open and close delete group modal', () => {
            ...
            expect(component.showDeleteGroupModal()).toBe(true);
            component.closeDeleteGroupModal();
            expect(component.showDeleteGroupModal()).toBe(false);
        });

        it('should delete group and invalidate cache', () => {
            mockCategoryAdmin.deleteCategoryGroup.mockReturnValue(of({ success: true }));
            // After delete, loadGroups returns empty so selectedEntityType stays undefined
            mockCategoryAdmin.getCategoryEntityTypes.mockReturnValue(of([]));
            component.openDeleteGroupModal();
