...
describe('PermissionsPage', () => {
    ...
    beforeEach(async () => {
        mockPermissionsService = {
            isAdmin: vi.fn().mockName("PermissionsService.isAdmin"),
            getRoles: vi.fn().mockName("PermissionsService.getRoles"),
            getTables: vi.fn().mockName("PermissionsService.getTables"),
            getRolePermissions: vi.fn().mockName("PermissionsService.getRolePermissions"),
            setRolePermission: vi.fn().mockName("PermissionsService.setRolePermission"),
            createRole: vi.fn().mockName("PermissionsService.createRole"),
            getEntityActionPermissions: vi.fn().mockName("PermissionsService.getEntityActionPermissions"),
            getEntityActionRoles: vi.fn().mockName("PermissionsService.getEntityActionRoles"),
            setEntityActionPermission: vi.fn().mockName("PermissionsService.setEntityActionPermission"),
            getRoleCanManage: vi.fn().mockName("PermissionsService.getRoleCanManage"),
            setRoleCanManage: vi.fn().mockName("PermissionsService.setRoleCanManage"),
            deleteRole: vi.fn().mockName("PermissionsService.deleteRole")
        };
        mockAuthService = {
        ...
        mockPermissionsService.isAdmin.mockReturnValue(of(true));
        mockPermissionsService.getRoles.mockReturnValue(of([
            { id: 1, display_name: 'Admin', role_key: 'admin' },
            { id: 2, display_name: 'Editor', role_key: 'editor' }
        ]));
        mockPermissionsService.getTables.mockReturnValue(of(['issues', 'users', 'reservations:notes']));
        mockPermissionsService.getRolePermissions.mockReturnValue(of([]));
    ...
    describe('Role Delegation Tab (v0.31.0)', () => {
        beforeEach(() => {
            ...
            mockPermissionsService.getRoleCanManage.mockReturnValue(of([
                { managed_role_id: 2, managed_role_name: 'Editor', managed_role_key: 'editor' }
            ]));
            mockPermissionsService.setRoleCanManage.mockReturnValue(of({ success: true }));
            mockPermissionsService.deleteRole.mockReturnValue(of({ success: true, message: 'Role deleted' }));
        });
        ...
        it('should call deleteRole with selected role ID', () => {
            ...
            mockPermissionsService.deleteRole.mockReturnValue(of({ success: true }));
            mockPermissionsService.getRoles.mockReturnValue(of([
                { id: 1, display_name: 'admin', role_key: 'admin' }
            ]));
