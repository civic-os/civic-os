...
describe('SchemaEditorPage - System Type Filtering', () => {
    ...
    beforeEach(async () => {
        mockSchemaService = {
            ...
            getDetectedJunctionTables: vi.fn().mockName("SchemaService.getDetectedJunctionTables")
        };
        mockThemeService = {
            isDark: vi.fn().mockName("ThemeService.isDark")
        };

        mockSchemaService.getEntities.mockReturnValue(of([]));
        mockSchemaService.getProperties.mockReturnValue(of([]));
        mockSchemaService.getDetectedJunctionTables.mockReturnValue(of(new Set<string>()));
...
describe('SchemaEditorPage - Port Reconnection Logic', () => {
    ...
    describe('link reconnection workflow', () => {
        ...
        it('should maintain perpendicular anchors during reconnection', () => {
            ...
            const mockLink = {
                source: vi.fn().mockName('source').mockReturnValue({ id: 'source', port: 'old_port' }),
                target: vi.fn().mockName('target').mockReturnValue({ id: 'target', port: 'old_port' })
            };
    ...
    describe('batching during reconnection', () => {
        it('should use batching to prevent multiple router recalculations', () => {
            ...
            const mockGraph = {
                startBatch: vi.fn().mockName('startBatch'),
                stopBatch: vi.fn().mockName('stopBatch'),
                getLinks: () => []
            };
...
describe('SchemaEditorPage - Keyboard Node Nudging (Task 6)', () => {
    ...
    function computeNudgeDelta(key: string, large: boolean): {
        dx: number;
        dy: number;
    } | null {
    ...
    describe('handleNodeKeydown()', () => {
        function createMockElement(x: number, y: number) {
            return {
                position: vi.fn().mockName('position').mockImplementation((newX?: number, newY?: number) => {
                    if (newX === undefined) {
                        return { x, y };
                    }
                    x = newX;
        ...
        beforeEach(() => {
            ...
            vi.spyOn(component as any, 'adjustVertices').mockResolvedValue(undefined);
        });
        ...
        it('moves the element position by the nudge delta and announces it', () => {
            ...
            vi.spyOn(evt, 'preventDefault').mockReturnValue(undefined);
            vi.spyOn(evt, 'stopPropagation').mockReturnValue(undefined);
        ...
        it('ignores non-arrow keys and does not move the element or announce', () => {
            ...
            expect(element.position).not.toHaveBeenCalledWith(expect.anything(), expect.anything());
