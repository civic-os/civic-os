...
describe('SystemPoliciesPage', () => {
    ...
    beforeEach(async () => {
        mockIntrospection = {
            getRlsPolicies: vi.fn().mockName("IntrospectionService.getRlsPolicies")
        };
        mockIntrospection.getRlsPolicies.mockReturnValue(of([
            createMockPolicy(),
    ...
    it('should toggle policy expansion', () => {
        ...
        expect(component.isExpanded(key)).toBe(false);
        component.togglePolicy(key);
        expect(component.isExpanded(key)).toBe(true);
    });
