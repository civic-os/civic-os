...
describe('SeriesGroupManagementPage', () => {
    ...
    function createTestBed(routeParams: any = {}) {
        mockRecurringService = {
            getSeriesGroups: vi.fn().mockName("RecurringService.getSeriesGroups"),
            getSeriesGroupDetail: vi.fn().mockName("RecurringService.getSeriesGroupDetail"),
            deleteSeriesGroup: vi.fn().mockName("RecurringService.deleteSeriesGroup"),
            describeRRule: vi.fn().mockName("RecurringService.describeRRule"),
            splitSeries: vi.fn().mockName("RecurringService.splitSeries"),
            updateSeriesGroupInfo: vi.fn().mockName("RecurringService.updateSeriesGroupInfo"),
            updateSeriesSchedule: vi.fn().mockName("RecurringService.updateSeriesSchedule"),
            updateSeriesTemplate: vi.fn().mockName("RecurringService.updateSeriesTemplate")
        };
        mockSchemaService = {
        ...
        mockRecurringService.getSeriesGroups.mockReturnValue(of(mockGroups));
        mockRecurringService.getSeriesGroupDetail.mockReturnValue(of(mockGroups[0]));
        mockRecurringService.describeRRule.mockReturnValue('Weekly');
    ...
    describe('ngOnInit', () => {
        ...
        it('should not load groups when read permission is denied', () => {
            mockAuthService.hasPermission.mockImplementation((_table: string, action: string) => {
                return false;
            });
    ...
    describe('selectGroup', () => {
        ...
        it('should set selected group ID and load detail', () => {
            ...
            expect(mockRouter.navigate).toHaveBeenCalledWith(['/admin/recurring-schedules', 1], { replaceUrl: true });
        });
    });
