...
describe('TimeSlotCalendarComponent', () => {
    ...
    it('should update FullCalendar when events change', async () => {
        ...
        const mockCalendarApi = {
            getEventSources: vi.fn().mockName('getEventSources').mockReturnValue([mockEventSource]),
            addEventSource: vi.fn().mockName('addEventSource'),
            render: vi.fn().mockName('render')
        };
    ...
    it('should emit eventClick when calendar event is clicked', () => {
        vi.spyOn(component.eventClick, 'emit').mockReturnValue(undefined);
    ...
    it('should emit dateSelect and valueChange in edit mode', () => {
        ...
        vi.spyOn(component.dateSelect, 'emit').mockReturnValue(undefined);
        vi.spyOn(component.valueChange, 'emit').mockReturnValue(undefined);
        ...
        expect(component.valueChange.emit).toHaveBeenCalledWith(expect.stringMatching(/\[2025-03-15T14:00:00.*,2025-03-15T16:00:00.*\)/));
    });
    ...
    describe('ViewChild Timing', () => {
        ...
        it('should not call FullCalendar API before view init', () => {
            const mockApi: any = {
                getEventSources: vi.fn().mockName("CalendarApi.getEventSources"),
                addEventSource: vi.fn().mockName("CalendarApi.addEventSource")
            };
            mockApi.getEventSources.mockReturnValue([]);
        ...
        it('should update events after view init', async () => {
            const mockApi: any = {
                getEventSources: vi.fn().mockName("CalendarApi.getEventSources"),
                addEventSource: vi.fn().mockName("CalendarApi.addEventSource"),
                setOption: vi.fn().mockName("CalendarApi.setOption"),
                changeView: vi.fn().mockName("CalendarApi.changeView"),
                gotoDate: vi.fn().mockName("CalendarApi.gotoDate"),
                getDate: vi.fn().mockName("CalendarApi.getDate")
            };
            mockApi.getEventSources.mockReturnValue([]);
            mockApi.getDate.mockReturnValue(new Date());
    ...
    describe('handleDatesSet guard (Bug Fix: back-navigation URL overwrite)', () => {
        it('should NOT emit dateRangeChange before viewInitialized', () => {
            ...
            vi.spyOn(component.dateRangeChange, 'emit').mockReturnValue(undefined);
    ...
    describe('Input-sync effect (Bug Fix: entity transition stale calendar)', () => {
        it('should call gotoDate when initialDate input changes after init', async () => {
            const mockApi: any = {
                setOption: vi.fn().mockName("CalendarApi.setOption"),
                changeView: vi.fn().mockName("CalendarApi.changeView"),
                gotoDate: vi.fn().mockName("CalendarApi.gotoDate"),
