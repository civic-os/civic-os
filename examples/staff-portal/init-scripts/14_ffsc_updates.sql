...
SET config = jsonb_build_object(
  'content', E'# Welcome to the FFSC Staff Portal\n\nThe Flint Freedom Schools Collaborative (FFSC) connects young people with culturally relevant education through a network of Freedom Schools sites across Flint, Michigan.\n\nThis portal helps our team manage day-to-day operations during the summer program — from onboarding and document tracking to time management and expense reimbursements.\n\n## Getting Started\n\nUse the **Staff Portal** dashboard (select from the dashboard icon above) to access your tasks, clock in/out, submit requests, and more.\n\n---\n\n*Powered by [Civic OS](https://civic-os.org)*',
  'enableHtml', false
)
