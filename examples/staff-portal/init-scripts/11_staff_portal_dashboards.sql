...
DECLARE
  ...
  v_re_pending_id INT;
  -- time_entry statuses (for nav_buttons query params)
...
BEGIN
  ...
  SELECT id INTO v_te_clock_in_id FROM metadata.statuses
    WHERE entity_type = 'time_entry' AND status_key = 'clock_in';
  SELECT id INTO v_te_clock_out_id FROM metadata.statuses
    WHERE entity_type = 'time_entry' AND status_key = 'clock_out';
