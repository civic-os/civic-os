...
interface CheckConstraint {
  ...
  constraint_type: 'enum' | 'range';
  enum_values?: string[];  // For IN constraints
  min_value?: number;      // For BETWEEN constraints
  max_value?: number;      // For BETWEEN constraints
}
...
interface MockDataConfig {
  ...
  dateRanges?: {
    [key: string]: {  // key format: "tableName.columnName"
...
class MockDataGenerator {
  ...
  private checkConstraintsMap: Map<string, CheckConstraint> = new Map();
  ...
  async fetchSchema() {
    ...
    await this.fetchCheckConstraints();
  }

  async fetchCheckConstraints() {
    if (!this.client) throw new Error('Database not connected');

    // Build list of entity table names for filtering
    const entityTableNames = new Set(this.entities.map(e => e.table_name));
    ...
    for (const row of result.rows) {
      // Only process constraints for our entity tables
      if (!entityTableNames.has(row.table_name)) {
        continue;
      }

      // Extract column name from constraint definition
      const columnName = this.extractColumnNameFromConstraint(row.constraint_definition);

      if (columnName) {
        const parsed = this.parseCheckConstraint(row.table_name, columnName, row.constraint_name, row.constraint_definition);

        if (parsed) {
          this.checkConstraints.push(parsed);
          const key = `${parsed.table_name}.${parsed.column_name}`;
          this.checkConstraintsMap.set(key, parsed);
        }
      }
    }

    console.log(`Fetched ${this.checkConstraints.length} CHECK constraints`);
  }

  private extractColumnNameFromConstraint(definition: string): string | null {
    // Remove "CHECK" wrapper and extract column name from constraint
    // Examples:
    //   "CHECK (((status)::text = ANY (ARRAY[...])))" → "status"
    //   "CHECK ((("position")::text = ANY ...))" → "position" (quoted reserved keyword)
    //   "CHECK (((season_year >= 2018) AND (season_year <= 2030)))" → "season_year"
    const innerDef = definition.replace(/^CHECK\s*\(/i, '').trim();

    // Pattern 1: (((column_name) or ((("column_name") with optional quotes
    let match = innerDef.match(/\(\(+"?(\w+)"?\)/);
    if (match) return match[1];

    // Pattern 2: Simple format without triple parens (rare)
    match = innerDef.match(/\((\w+)\s*(::|>=|<=|=|IN|BETWEEN)/i);
    if (match) return match[1];

    return null;
  }

  private parseCheckConstraint(tableName: string, columnName: string, constraintName: string, definition: string): CheckConstraint | null {
    // Remove "CHECK" wrapper
    const innerDef = definition.replace(/^CHECK\s*\(/i, '').replace(/\)$/, '').trim();

    // Parse PostgreSQL IN constraint format: "((column_name)::text = ANY ((ARRAY['value1', 'value2'])::text[]))"
    const anyArrayMatch = innerDef.match(/ARRAY\[(.*?)\]/);
    if (anyArrayMatch) {
      const enumValues = anyArrayMatch[1]
        .split(',')
        .map(v => {
          // Extract value from "'value'::character varying" format
          const valueMatch = v.match(/'([^']+)'/);
    ...
    // Parse range constraint format: "((column_name >= min) AND (column_name <= max))"
    const rangeMatch = innerDef.match(/(\w+)\s*>=\s*(\d+)\)\s*AND\s*\(\1\s*<=\s*(\d+)/i);
    if (rangeMatch) {
      return {
        table_name: tableName,
        column_name: columnName,
        ...
        min_value: parseInt(rangeMatch[2]),
        max_value: parseInt(rangeMatch[3]),
      };
    }

    // Constraint pattern not recognized
    return null;
  }
  ...
  private getCheckConstraint(tableName: string, columnName: string): CheckConstraint | undefined {
    const key = `${tableName}.${columnName}`;
    return this.checkConstraintsMap.get(key);
  }
  ...
  private generateDisplayName(tableName: string): string {
    switch (tableName) {
      case 'participants': {
        // Youth soccer participants: full person names
        return faker.person.fullName();
      }

      case 'teams': {
        // Team names: "{Age Group} - {Color}"
  ...
  private generateFakeValue(prop: SchemaEntityProperty, relatedIds?: any[]): any {
    ...
    switch (type) {
      case EntityPropertyType.TextShort:
        ...
        // Check for enum CHECK constraint (IN clause)
        const enumConstraint = this.getCheckConstraint(prop.table_name, prop.column_name);
        if (enumConstraint && enumConstraint.constraint_type === 'enum' && enumConstraint.enum_values) {
          return faker.helpers.arrayElement(enumConstraint.enum_values);
        }
      ...
      case EntityPropertyType.Date: {
        // Check for custom date range configuration
        const dateRangeKey = `${prop.table_name}.${prop.column_name}`;
        if (this.config.dateRanges && this.config.dateRanges[dateRangeKey]) {
          const range = this.config.dateRanges[dateRangeKey];
      ...
      case EntityPropertyType.IntegerNumber:
        // Check for range CHECK constraint (BETWEEN clause)
        const rangeConstraint = this.getCheckConstraint(prop.table_name, prop.column_name);
        if (rangeConstraint && rangeConstraint.constraint_type === 'range') {
          return faker.number.int({
            min: rangeConstraint.min_value!,
            max: rangeConstraint.max_value!
          });
        }

        // Apply min/max constraints from validation rules
        let minInt = 1;
        let maxInt = 1000;
        if (minRule && minRule.validation_value) {
