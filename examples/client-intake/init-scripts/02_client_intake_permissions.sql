...
-- =====================================================
-- CUSTOM ROLE: ic_staff
-- =====================================================

INSERT INTO metadata.roles (role_key, display_name, description)
VALUES ('ic_staff', 'IC Staff', 'International Center staff with full operational access')
...
-- -----------------------------------------------
-- service_categories: anonymous=read, user=read, ic_staff=all, admin=all
-- -----------------------------------------------
...
WHERE p.table_name = 'service_categories'
  ...
  AND r.role_key IN ('anonymous', 'user', 'ic_staff', 'admin')
...
WHERE p.table_name = 'clients'
  ...
  AND r.role_key IN ('user', 'ic_staff', 'admin')
ON CONFLICT DO NOTHING;
...
-- -----------------------------------------------
-- partners: anonymous=read, user=read, ic_staff=all, admin=all
-- -----------------------------------------------
...
-- -----------------------------------------------
-- referrals: ic_staff=all, admin=all
...
-- -----------------------------------------------
-- follow_up_surveys: ic_staff=all, admin=all
...
-- Grant notes access to ic_staff and admin only
INSERT INTO metadata.permission_roles (permission_id, role_id)
SELECT p.id, r.id
FROM metadata.permissions p
CROSS JOIN metadata.roles r
