...
-- 02: Consent Subsystem
-- =====================================================
-- Runs AFTER the reskinned ICGF base script (01_*).
...
--
-- Items marked -- VERIFY must be checked against the actual
-- base script in the repo before running.
-- =====================================================
...
VALUES ('consent_method', 'How the client''s consent was captured')
...
CREATE TABLE public.client_consents (
  ...
  expires_date  DATE,
  revoked_date  DATE,
  captured_by   UUID            REFERENCES metadata.civic_os_users(id), -- VERIFY: match base script user-FK target
  evidence_file UUID            REFERENCES metadata.files(id),          -- optional evidence, not the mechanism
...
-- VERIFY: trigger function names match base script conventions
CREATE TRIGGER client_consents_created_at BEFORE INSERT ON public.client_consents
  FOR EACH ROW EXECUTE FUNCTION metadata.set_created_at();
CREATE TRIGGER client_consents_updated_at BEFORE UPDATE ON public.client_consents
...
ALTER TABLE public.clients
  ADD COLUMN consent_state TEXT NOT NULL DEFAULT 'none'
    CHECK (consent_state IN ('none', 'pending', 'active', 'expired', 'revoked')),
...
BEGIN
  ...
  UPDATE clients
  SET consent_state = v_state, consent_note = v_note
  WHERE id = p_client_id
    AND (consent_state IS DISTINCT FROM v_state OR consent_note IS DISTINCT FROM v_note);
END;
$$;
...
BEGIN
  IF NOT has_rpc_permission('record_client_consent') THEN
  ...
  VALUES (p_entity_id, get_status_id('client_consent', 'active'), p_method_id,
          p_granted_date, v_expires, current_user_id(), p_evidence); -- VERIFY: current_user_id() helper name
...
DECLARE
  ...
  v_consent_id BIGINT;
BEGIN
  IF NOT has_rpc_permission('request_client_consent') THEN
    RETURN jsonb_build_object('success', false, 'message', 'Permission denied');
  END IF;

  SELECT * INTO v_client FROM clients WHERE id = p_entity_id;
  -- VERIFY: clients.email column name and clients.user_id portal-account link in base script
...
-- Gate the existing Refer Client action.
-- VERIFY: action_name in base script ('refer_client' assumed).
UPDATE metadata.entity_actions
SET enabled_condition = '{"field": "consent_state", "operator": "eq", "value": "active"}'::jsonb,
    disabled_tooltip  = 'Referral requires active consent; see Consent Status on this record.'
WHERE table_name = 'clients' AND action_name = 'refer_client';
...
VALUES ('clients', 'record_consent', 'Record Consent',
    ...
    'record_client_consent', 'verified_user', 'success', 20, FALSE);

INSERT INTO metadata.entity_action_params (entity_action_id, param_name, display_name, param_type, required, sort_order, category_entity_type, default_value, placeholder)
SELECT ea.id, x.param_name, x.display_name, x.param_type, x.required, x.sort_order, x.category_entity_type, x.default_value, x.placeholder
FROM metadata.entity_actions ea,
(VALUES
  ('p_method_id',    'Consent Method',  'category', TRUE,  10, 'consent_method', NULL, NULL),
  ('p_granted_date', 'Granted Date',    'date',     TRUE,  20, NULL, CURRENT_DATE::TEXT, NULL),
  ('p_expires_date', 'Expires Date',    'date',     FALSE, 30, NULL, NULL, 'Defaults to one year from granted date'),
  ('p_evidence',     'Evidence (optional)', 'file', FALSE, 40, NULL, NULL, NULL)
) AS x(param_name, display_name, param_type, required, sort_order, category_entity_type, default_value, placeholder)
WHERE ea.table_name = 'clients' AND ea.action_name = 'record_consent';
-- VERIFY: entity_action_params column list (file_type for the file param) against base script examples.

-- Request Consent (fires notification; sits beside the gated button)
INSERT INTO metadata.entity_actions (table_name, action_name, display_name, description, rpc_function,
    icon, button_style, sort_order, requires_confirmation, confirmation_message, visibility_condition)
VALUES ('clients', 'request_consent', 'Request Consent',
    'Email the client a request to confirm consent for referrals.',
    'request_client_consent', 'mark_email_unread', 'secondary', 30, TRUE,
    'Send this client a consent request by email?',
    '{"field": "consent_state", "operator": "ne", "value": "active"}'::jsonb);
...
-- Rehearsal: SELECT trigger_scheduled_job('consent_maintenance');

-- =====================================================
-- 10. NOTIFICATION TEMPLATES
-- =====================================================
-- VERIFY: notification_templates column list in base script.
...
-- 11. ENTITY METADATA
-- =====================================================
-- VERIFY: metadata.entities and metadata.properties column
-- lists against base script insert patterns.

INSERT INTO metadata.entities (table_name, display_name, description, sort_order)
VALUES ('client_consents', 'Client Consents',
        'Consent records with expiration; the referral gate reads these.', 35);

-- Properties: label and order the consent record fields; on
-- clients, expose consent_state and consent_note as read-only
-- (excluded from create/edit), placed adjacent to status so
-- the disabled Refer Client button has its visible reason.
INSERT INTO metadata.properties (table_name, column_name, display_name, sort_order) VALUES
  ('client_consents', 'client_id',     'Client',         10),
  ('client_consents', 'status_id',     'Status',         20),
  ('client_consents', 'method_id',     'Method',         30),
  ('client_consents', 'granted_date',  'Granted',        40),
  ('client_consents', 'expires_date',  'Expires',        50),
  ('client_consents', 'revoked_date',  'Revoked',        60),
  ('client_consents', 'captured_by',   'Captured By',    70),
  ('client_consents', 'evidence_file', 'Evidence',       80),
  ('clients',         'consent_state', 'Consent State',  55),
  ('clients',         'consent_note',  'Consent Status', 56);
-- VERIFY: read-only / hide-on-create flags for consent_state and consent_note.
...
CREATE POLICY client_consents_read ON public.client_consents FOR SELECT TO authenticated
  USING (
    has_permission('read', 'client_consents')
...
CREATE POLICY client_consents_create ON public.client_consents FOR INSERT TO authenticated
  WITH CHECK (has_permission('create', 'client_consents'));
CREATE POLICY client_consents_update ON public.client_consents FOR UPDATE TO authenticated
  USING (has_permission('update', 'client_consents'));
...
-- 13. SCHEMA DECISIONS
-- =====================================================
-- Init-script context: direct INSERT, not the RPC.
...
(ARRAY['clients', 'client_consents'],
 ...
 'clients.consent_state and clients.consent_note are trigger-maintained from client_consents; Refer Client is enabled only when consent_state = active; the dated reason displays as a read-only property.',
