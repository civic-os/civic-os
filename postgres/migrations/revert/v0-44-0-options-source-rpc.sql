-- Revert civic_os:v0-44-0-options-source-rpc from pg

BEGIN;

-- 1. DROP helper RPC
DROP FUNCTION IF EXISTS public.get_m2m_options_source_rpcs();

-- 2. DROP dependent VIEW first (schema_entity_dependencies references schema_properties)
-- Save for recreation after schema_properties is restored.
...
-- 3. REMOVE new columns from metadata.properties
ALTER TABLE metadata.properties DROP COLUMN IF EXISTS options_source_rpc;
ALTER TABLE metadata.properties DROP COLUMN IF EXISTS depends_on_columns;

-- 4. RECREATE schema_properties VIEW (v0.34.0 version, without options_source_rpc columns)
...
-- 5. RECREATE schema_entity_dependencies VIEW (v0.33.0 version)
-- This VIEW depends on schema_properties and was dropped in step 1.
