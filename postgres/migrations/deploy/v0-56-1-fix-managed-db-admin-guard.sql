-- Deploy v0-56-1-fix-managed-db-admin-guard
-- Fix admin guard for managed database compatibility (DigitalOcean, AWS RDS, Google Cloud SQL)
--
-- Problem: The inline admin guard in register_guided_form(), add_guided_form_step(),
-- and grant_guided_form_permissions() checks `current_setting('is_superuser') = 'on'`,
-- but managed DB admin users (doadmin, rds_admin, etc.) are NOT true superusers.
-- Combined with no JWT in psql sessions, the guard rejects legitimate admin operations
-- and the WHEN OTHERS handler swallows the error silently.
--
-- Solution: A reusable _is_db_admin() helper that also checks for CREATEROLE privilege,
-- which all managed DB admin users have but application roles do not.

BEGIN;

-- ============================================================================
-- 1. Helper: metadata._is_db_admin()
-- ============================================================================
-- Single source of truth for "is this session an admin?" across all contexts:
--   - App-level JWT admin (via is_admin())
--   - True PostgreSQL superuser (native installs, docker)
--   - Managed DB admin (DigitalOcean doadmin, AWS rds_admin, etc.)
--
-- Placed in metadata schema: internal helper, not exposed via PostgREST.
-- SECURITY DEFINER: allows reading pg_roles regardless of caller's privileges.

CREATE OR REPLACE FUNCTION metadata._is_db_admin()
RETURNS BOOLEAN AS $$
  SELECT public.is_admin()                                          -- App-level JWT admin
    OR current_setting('is_superuser', true) = 'on'                 -- True PG superuser
    OR EXISTS (                                                      -- Managed DB admin
        SELECT 1 FROM pg_roles
        WHERE rolname = current_user AND rolcreaterole
    );
$$ LANGUAGE sql STABLE SECURITY DEFINER;

COMMENT ON FUNCTION metadata._is_db_admin() IS
    'Check if current session has admin privileges: JWT admin, PG superuser, or managed DB admin (CREATEROLE). '
    'Internal helper for framework functions. Added in v0.56.1.';


-- ============================================================================
-- 2. Redefine register_guided_form() — replace inline guard
-- ============================================================================
...
DECLARE
    ...
    v_gf_col    NAME;
BEGIN
    -- Admin guard: only admins can register guided forms
    IF NOT metadata._is_db_admin() THEN
...
COMMENT ON FUNCTION public.register_guided_form(NAME, NAME, TEXT, NAME, VARCHAR, TEXT, BOOLEAN, NAME, NAME) IS
    ...
    'v0.56.1: use metadata._is_db_admin() for managed DB compatibility.';


-- ============================================================================
-- 3. Redefine add_guided_form_step() — replace inline guard
-- ============================================================================
...
DECLARE
    ...
    v_gf_col           NAME;
BEGIN
    -- Admin guard: only admins can add guided form steps
    IF NOT metadata._is_db_admin() THEN
        RAISE EXCEPTION 'Admin access required';
    END IF;
...
-- ============================================================================
-- 4. Redefine grant_guided_form_permissions() — replace inline guard
-- ============================================================================
...
DECLARE
    ...
    v_skipped      INT := 0;
BEGIN
    -- Admin guard: only admins can grant guided form permissions
    IF NOT metadata._is_db_admin() THEN
        RAISE EXCEPTION 'Admin access required';
    END IF;
...
COMMENT ON FUNCTION public.grant_guided_form_permissions(NAME, INTEGER, TEXT[]) IS
    ...
    'Child step tables inherit RBAC from the parent. '
    'v0.56.1: use metadata._is_db_admin() for managed DB compatibility.';
