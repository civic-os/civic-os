-- Deploy v0-20-2-auto-create-permissions
-- Auto-create missing permission entries in set_role_permission RPC
--
-- This fixes a usability issue where new tables don't appear in the
-- Permissions management UI until they have entries in metadata.permissions.
-- Now, set_role_permission will auto-create the permission entry if missing.

BEGIN;

-- Enhanced set_role_permission that auto-creates missing permission entries
...
-- Helper function to ensure all CRUD permissions exist for a table
-- Useful for integrators to call when setting up new tables
...
GRANT EXECUTE ON FUNCTION public.ensure_table_permissions(TEXT) TO authenticated;
