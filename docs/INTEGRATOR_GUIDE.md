...
**UI**: User Management page at `/admin/users` (admin-only). Features search, status filter, role filter, create modal with role selection, edit user info, retry for failed provisioning, and bulk import from Excel.
