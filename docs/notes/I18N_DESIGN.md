...
- **Notification template translation** — Templates use Go template syntax with JSONB entity snapshots. The `t()` function can resolve per-locale template text if the Go worker sets `request.header.accept-language` before querying. Entity *data* in notifications (addresses, names) is user-entered content, not translatable. **Prerequisite**: the locale persistence gap (below) must be fixed first so the worker has a reliable locale for each recipient.
- **Locale persistence gap** — `civic_os_users_private.locale` is only written when a user explicitly picks a language in Settings > Language (via `set_user_locale()` RPC). Three sources that *could* write locale currently do not:
  1. **Browser language auto-persist**: `resolveInitialLocale()` detects `navigator.language` and uses it for the Angular signal, but never persists it to the DB. Fix: on first authenticated page load, if `locale IS NULL` in the user's profile, call `set_user_locale()` with the resolved browser locale. One-time backfill per user.
  2. **`refresh_current_user()` login sync**: Syncs `display_name`, `email`, `phone`, and roles from JWT on every login — but ignores Keycloak's `locale` user attribute. Fix: add a `locale` JWT mapper in Keycloak config and read it in `refresh_current_user()`.
  3. **`initFromJwt()` dead code**: `LocaleService.initFromJwt()` was built to read a locale JWT claim and override the Angular signal, but is never called from production code (only mocked in tests). Fix: call it from `AppComponent` or `AuthService` after JWT decode.
  The simplest fix is option 1 (browser auto-persist) — no Keycloak config changes needed. This should be implemented alongside notification template translation so the Go worker always has a locale to query.
