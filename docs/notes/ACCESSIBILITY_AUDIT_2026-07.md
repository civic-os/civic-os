...
- **Per-example sweep series (2026-07-11/12, all 7 examples):** each example's schema was rendered live and asserted. Fixes shipped: unlabeled M:M/multi-select search inputs (broader-impacts), entity-notes textareas + link button (community-center), TimeSlot/recurring-series label associations (mottpark), guided-form current-step `aria-current` + save/progress live regions + review-section names + universal sr-only required markers (neighborhood-hub), app-wide admin/editor label association + admin table captions/scopes + perceivable swatches (staff-portal), geoman toolbar button names (storymap). Remaining low-priority residuals from the sweeps: dashboard map markers/clusters have generic accessible names ("Marker", count-only clusters — could carry record names); raw WKT `_text` columns render on lists (display nit); markdown-widget h1 duplication and inverse-relationship 400s filed as separate follow-up tasks.
- **Drawer toggle is an invisible tab stop (found in the 2026-07-11 live keyboard pass):** the DaisyUI `drawer-toggle` checkbox is focusable but visually hidden — a keyboard user hits a tab stop with no visible focus indicator, and its accessible name is a concatenation of its three `<label>`s ("Open navigation menu close sidebar Close navigation menu"). It *works* (Space opens the drawer; sidebar links then appear at the end of the tab order), but the shell should use a real `<button aria-expanded aria-controls>` and give the checkbox `tabindex="-1"`. Moderate; keyboard-functional today.
