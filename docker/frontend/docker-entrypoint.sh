...
window.civicOsConfig = {
  ...
  pwa: {
    ...
    appName: '$(echo "${PWA_APP_NAME}" | sed "s/'/\\\\'/g")' || ''
  }
};
</script>
EOF
...
if [ "$PWA_ENABLED" = "true" ]; then
  ...
  sed -i "s|PWA_APP_TITLE_PLACEHOLDER|$(echo "${PWA_APP_NAME:-${APP_TITLE:-Civic OS}}" | sed "s|&|\\\\&|g")|g" /usr/share/nginx/html/manifest.webmanifest
