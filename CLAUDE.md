...
**Entity Action Buttons** (v0.18.0+): Metadata-driven action buttons on Detail pages that execute PostgreSQL RPC functions. **Action Parameters** (v0.32.0+) allow user-provided form fields in modals. **Filtered Action Param Dropdowns** (v0.54.0+): FK action params support `options_source_rpc` for custom RPC-driven option lists and `depends_on_params` for cascading dropdowns between sibling params — mirrors the property-level `options_source_rpc` + `depends_on_columns` pattern. **Photo Gallery Action Params** (v0.55.0+): `photo_gallery` param type embeds `PhotoGalleryEditorComponent` in action modals with `target_column` mapping to an entity gallery column for config lookup. Gallery always starts blank; RPC receives draft gallery UUID. **Dot-notation conditions** (v0.53.2+): `visibility_condition` and `enabled_condition` support dot-notation for embedded FK properties (e.g., `"field": "status_id.status_key"`) — prefer `status_key` over numeric IDs for portability across environments. **Maintenance note**: When adding a new `EntityPropertyType`, check if it should also be added as an action param type. See `docs/INTEGRATOR_GUIDE.md` (Entity Action Buttons section) and `docs/development/ENTITY_ACTIONS.md` for details.
...
**Guided Forms** (v0.48.0+): Multi-step form workflows with conditional skip/require, auto-save, review-and-submit, and lock-on-submit. Configure via `guided_form_key` on `metadata.entities` with steps in `metadata.guided_form_steps`. **Dual-Status Pattern** (v0.55.2+): The framework-managed lifecycle column is `guided_form_status_id` (draft → complete → submitted). Integrators add a separate `status_id` (or `workflow_status_id`) column for business workflow (pending → approved → denied). The `on_submit_rpc` bridges the two by setting the business status. See `docs/notes/GUIDED_FORM_SYSTEM_DESIGN.md` for architecture and `docs/INTEGRATOR_GUIDE.md` (Guided Forms section) for setup.
...
**Full-Text Search + Substring Search** (v0.55.2+): Two explicit metadata columns control List page search behavior: `fulltext_search_column` (tsvector column for FTS via `wfts`) and `substring_search_column` (column for ILIKE substring search via `pg_trgm`). When both are set, the frontend constructs a hybrid `or=()` PostgREST query. Search input shows when either column is set (backward compat: `search_fields` alone still works). The `search_fields` array is preserved for documentation. See `docs/INTEGRATOR_GUIDE.md` (Full-Text Search section) and `docs/notes/HYBRID_SEARCH_DESIGN.md` for architecture.
...
**Multi-Language (i18n)** (v0.57.0+): Framework UI strings render in the user's preferred language. Phase 1 covers ~250 Angular UI keys with English/Spanish support. Architecture: `LocaleService` (signal-based locale), `TranslationService` (DB fetch + bundled English fallback), `TranslatePipe` (`{{ 'key' | translate }}`), locale HTTP interceptor (`Accept-Language` header), `metadata.translations` table, `metadata.t()` lookup function. Language tab appears in Settings when `supportedLocales` has 2+ entries. English short-circuits with zero DB overhead. **Phase 2** (v0.58.0+): Instance metadata translations — 7 public VIEWs (`schema_entities`, `schema_properties`, `statuses`, `categories`, `static_text`, `schema_entity_actions`, `schema_guided_form_steps`) wrapped with `metadata.t()` on ~20 translatable columns. Source types: `entity`, `property`, `status`, `category`, `static_text`, `action`, `action_param`, `guided_form_step`. Frontend clears schema and dashboard caches on locale change via `effect()` watching `LocaleService.locale()`. See `docs/INTEGRATOR_GUIDE.md` (Multi-Language section) and `docs/notes/I18N_DESIGN.md` for architecture.
