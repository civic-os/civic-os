...
**File Storage Types** (`FileImage`, `FilePDF`, `File`): S3-based file storage with automatic thumbnail generation via consolidated worker. **File Administration** (v0.39.0+): Admin page at `/admin/files` for browsing all files. See `docs/development/FILE_STORAGE.md` for implementation guide and `docs/notes/ADMIN_PAGE_PITFALLS.md` for architecture patterns. **Signed File Access URLs** (designed, not yet built): In-database SigV4 presigned URL generation via `pgcrypto` in the `public.files` VIEW, enforcing RLS before granting file access. See `docs/notes/SIGNED_FILE_URLS_DESIGN.md` for architecture and `docs/notes/SIGV4_SIGNING_REFERENCE.md` for the signing algorithm.
...
**Anonymous Public Submit** (planned): Unauthenticated form submission for civic intake (service requests, volunteer signups, permit applications). Single `submit_public_form()` SECURITY DEFINER wrapper with three integrator tiers (metadata-only, validation hook, custom submit). Built-in anti-abuse: honeypot, timing, rate limiting, ALTCHA proof-of-work (verified in SQL via `pgcrypto`). See `docs/notes/ANONYMOUS_PUBLIC_SUBMIT_DESIGN.md` for architecture.
...
**MCP Server**: Semantic API adapter over PostgREST for LLM integration via Model Context Protocol. Provides 12 tools and 2 resources with display-name resolution, comprehensive error handling, and JWT auth + RLS. See `tools/mcp-server/README.md` for setup and `docs/notes/MCP_SERVER_DESIGN.md` for architecture.

**Transport** (v0.72.1+): Supports stdio (default for CLI) and HTTP Streamable (default in Docker via `MCP_TRANSPORT=http`). Per-request auth with self-contained OAuth discovery via `KEYCLOAK_URL`/`KEYCLOAK_REALM` env vars. Deploy behind Caddy with `handle_path /_/mcp/*`. See `docs/AUTHENTICATION.md` (Step 9) for full OAuth setup and `docs/notes/MCP_OAUTH_COMPATIBILITY_GUIDE.md` for spec compliance.

**Instance Context**: `MCP_SERVER_INSTRUCTIONS` env var (or `--instructions` CLI flag) prepends deployer-defined context before the generic usage instructions sent to LLM clients. Also prepended to the `civicos://schema/overview` resource. See `docs/notes/MCP_SERVER_DESIGN.md` (Instance Context section).

**Key modules**: `postgrest-client.ts` (typed HTTP client), `schema-cache.ts` (version-based refresh), `name-resolver.ts` (display name → identifier), `select-builder.ts` (PostgREST FK embedding), `http.ts` (HTTP server, Bearer extraction, OAuth metadata). Bun-based Docker container at `docker/mcp-server/Dockerfile` (~97 MB image, ~30 MB RAM).

**Action Condition Evaluation**: Dot-notation visibility/enabled conditions on actions (e.g., `status_id.status_key`). See `docs/notes/MCP_SERVER_DESIGN.md` (Action Condition Evaluation section).

**Name Resolution Paths**: Status/Category use entity_type-scoped lookups; FK columns use async PostgREST queries. See `docs/notes/MCP_SERVER_DESIGN.md` (Name Resolution Paths section).

**Development**: `cd tools/mcp-server && npm install && npm run build && npm test` (vitest). Full-stack integration tests: `cd tests/integration && ./run-tests.sh --ci` (real PostgreSQL + PostgREST + Keycloak).
