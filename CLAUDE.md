...
**Many-to-Many Relationships**: Auto-detected from junction tables. **CRITICAL**: Junction tables MUST use composite primary keys (NOT surrogate IDs). Renders on Detail pages with `ManyToManyEditorComponent`. Requires CREATE/DELETE permissions on junction table. Supports `options_source_rpc` for filtered M:M option lists (v0.44.0). Optionally renders as searchable split-panel modal via `fk_search_modal` (v0.45.0+) for large option sets with search, sort, filter, and pagination. **Inline positioning** via `show_inline` (v0.46.0) renders M:M in the property grid instead of bottom card, with buffered save on Edit/Create pages via `SaveProgressComponent`. M:M metadata flags are batch-loaded from `schema_m2m_properties` VIEW during `SchemaService.getProperties()` enrichment. **Rich Junction M:M** (v0.51.0+): Junction tables with extra columns beyond the two FKs are detected as "rich" junctions — extra columns render as editable fields in the M:M editor. Grandparent M:M detection via composable parent hops supports deeper relationship chains. See `docs/notes/MANY_TO_MANY_DESIGN.md` for implementation details, `docs/notes/FK_SEARCH_MODAL_DESIGN.md` (M:M Search Modal Design section) for split modal architecture, and `docs/INTEGRATOR_GUIDE.md` (M:M Search Modal and Inline M:M sections) for configuration.
