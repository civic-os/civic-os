...
**MCP Server**: Semantic API adapter over PostgREST for LLM integration via Model Context Protocol. Provides 11 tools (list_entities, describe_entity, list_actions, list_records, get_record, search, create_record, update_record, execute_action, add_note, get_status_workflow) and 2 resources (schema overview, entity detail). Resolves display names to identifiers at every level (entities, columns, FK values, statuses, actions). ETag-based optimistic concurrency prevents stale LLM overwrites. Uses the same JWT auth + RLS as the web frontend. See `tools/mcp-server/README.md` for setup and `docs/notes/MCP_SERVER_DESIGN.md` for architecture.

**Key modules**: `postgrest-client.ts` (typed HTTP client), `schema-cache.ts` (version-based refresh), `name-resolver.ts` (display name → identifier), `select-builder.ts` (PostgREST FK embedding). Bun-based Docker container at `docker/mcp-server/Dockerfile` (~97 MB image, ~30 MB RAM).

**Development**: `cd tools/mcp-server && npm install && npm run build && npm test` (384 unit tests via vitest). Full-stack integration tests: `cd tests/integration && ./run-tests.sh --ci` (real PostgreSQL + PostgREST + Keycloak).
