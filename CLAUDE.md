...
**MCP Server**: Semantic API adapter over PostgREST for LLM integration via Model Context Protocol. Provides 12 tools (list_entities, describe_entity, list_actions, list_records, get_record, search, create_record, update_record, execute_action, add_note, list_notes, get_status_workflow) and 2 resources (schema overview, entity detail). Resolves display names to identifiers at every level (entities, columns, FK values, statuses, actions). Comprehensive error handling translates Postgres constraint violations, type errors, and not-null violations to user-friendly messages with available values listed. Uses the same JWT auth + RLS as the web frontend. See `tools/mcp-server/README.md` for setup and `docs/notes/MCP_SERVER_DESIGN.md` for architecture.

**Transport** (v0.72.1+): Supports stdio (default for CLI) and HTTP Streamable (default in Docker via `MCP_TRANSPORT=http`). HTTP mode uses `createMcpHandler()` from MCP SDK v2.0.0 with per-request auth — each request's Bearer token creates a fresh PostgRESTClient+NameResolver while sharing the SchemaCache. Self-contained OAuth discovery via `KEYCLOAK_URL`/`KEYCLOAK_REALM` env vars serves both RFC 9728 (`oauth-protected-resource`) and RFC 8414 (`oauth-authorization-server`, cached from Keycloak OIDC config) — routing layer only needs to forward `/.well-known/oauth-*` to MCP server. Health check at `GET /health`. CORS enabled (including 2026-era `Mcp-Method`/`Mcp-Name` headers). 401 challenges use RFC 6750 §3 format with `error="invalid_token"`, `resource_metadata`, and `scope="mcp:tools"`. PRM includes `scopes_supported: ["mcp:tools"]` and `resource_name`. Redirect URIs cover Claude Desktop (`https://claude.ai/api/mcp/auth_callback`), Claude Code (RFC 8252 loopback), and generic MCP clients. DCR uses Consent Required policy (not Trusted Hosts) for compatibility with Anthropic's proxy-based registration. Deploy behind Caddy with `handle_path /_/mcp/* { reverse_proxy mcp-server:3001 }`. See `docs/AUTHENTICATION.md` (Step 9) for full setup and `docs/notes/MCP_OAUTH_COMPATIBILITY_GUIDE.md` for spec compliance reference.

**Key modules**: `postgrest-client.ts` (typed HTTP client), `schema-cache.ts` (version-based refresh with `baseUrl` exposure for per-session clients), `name-resolver.ts` (display name → identifier), `select-builder.ts` (PostgREST FK embedding), `http.ts` (HTTP server: Bun.serve / Node fallback, Bearer extraction, health, CORS, OAuth metadata). Bun-based Docker container at `docker/mcp-server/Dockerfile` (~97 MB image, ~30 MB RAM).

**Action Condition Evaluation**: `execute_action` fetches the current record with full FK embedding (via `buildSelectString`) before evaluating `visibility_condition` and `enabled_condition`. This enables dot-notation paths like `status_id.status_key` in conditions. Failed conditions return diagnostic messages showing actual vs expected values and suggest `get_record` to see available actions. See `docs/notes/MCP_SERVER_DESIGN.md` (Action Condition Evaluation section).

**Name Resolution Paths**: Status and Category columns use synchronous entity_type-scoped lookups (`resolveStatus`/`resolveCategory`), while FK columns use async PostgREST queries (`resolveForeignKeyValue`). This distinction matters because the shared `statuses` table uses an `entity_type` discriminator — "Active" in one entity is a different status than "Active" in another. See `docs/notes/MCP_SERVER_DESIGN.md` (Name Resolution Paths section).

**Development**: `cd tools/mcp-server && npm install && npm run build && npm test` (424 unit tests via vitest). Full-stack integration tests: `cd tests/integration && ./run-tests.sh --ci` (real PostgreSQL + PostgREST + Keycloak).
