...
**Transport** (v0.72.1+): Supports stdio (default for CLI) and HTTP Streamable (default in Docker via `MCP_TRANSPORT=http`). HTTP mode uses `createMcpHandler()` from MCP SDK v2.0.0 with per-request auth — each request's Bearer token creates a fresh PostgRESTClient+NameResolver while sharing the SchemaCache. Optional OAuth discovery via `KEYCLOAK_URL`/`KEYCLOAK_REALM` env vars serves RFC 9728 metadata at `/.well-known/oauth-protected-resource`. Health check at `GET /health`. CORS enabled. Deploy behind Caddy with `handle_path /_/mcp/* { reverse_proxy mcp-server:3001 }`. See `docs/AUTHENTICATION.md` (Step 9) for Keycloak `civic-os-mcp` client setup.
