#!/usr/bin/env node
/**
 * Browser Test: Workflow Skip Conditions + Review Submit
 *
 * Validates two specific Phase 1 fixes:
 * 1. Skip conditions are evaluated in WorkflowNavComponent (steps hidden when skip_if matches)
 * 2. WorkflowReviewSection Submit button is wired in DetailPage
 */
...
-- Add skip condition: step is skipped when is_nonprofit = true
INSERT INTO metadata.workflow_step_conditions (workflow_step_id, condition_type, field, operator, value, sort_order)
SELECT ws.id, 'skip_if', 'is_nonprofit', 'eq', 'true', 0
FROM metadata.workflow_steps ws
WHERE ws.workflow_key = 'browser_test_workflow' AND ws.step_key = 'details';

-- Seed a nonprofit parent (step should be skipped)
INSERT INTO public.browser_test_parent (id, display_name, workflow_status, is_nonprofit)
VALUES (99901, 'Nonprofit Test', 'complete', TRUE);
INSERT INTO metadata.workflow_progress (workflow_key, parent_id, step_key, completed_at)
VALUES ('browser_test_workflow', 99901, '__parent__', NOW())
ON CONFLICT (workflow_key, parent_id, step_key) DO UPDATE SET completed_at = NOW();

-- Seed a for-profit parent (step should be visible)
INSERT INTO public.browser_test_parent (id, display_name, workflow_status, is_nonprofit)
VALUES (99902, 'For-Profit Test', 'complete', FALSE);
INSERT INTO public.browser_test_step (display_name, workflow_status, parent_id)
VALUES ('Step Data', 'complete', 99902);
INSERT INTO metadata.workflow_progress (workflow_key, parent_id, step_key, completed_at)
...
async function runTests() {
  console.log('━━━ Workflow Conditions + Submit Tests ━━━');
  ...
  try {
    ...
    // === TEST 1: Skip condition hides step when is_nonprofit = true ===
    console.log('\n  --- Skip Condition Test ---');

    await page.goto(`${BASE_URL}/view/browser_test_parent/99901`);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);

    const navNonprofit = page.locator('app-workflow-nav');
    assert('Workflow nav appears for nonprofit parent', await navNonprofit.count() > 0);

    if (await navNonprofit.count() > 0) {
      const navText = await navNonprofit.textContent();
      const hasDetailsStep = navText.includes('Additional Details');
      assert('Skip condition hides step for nonprofit parent', !hasDetailsStep,
        `Nav text: ${navText?.substring(0, 200)}`);
    }

    // Test non-skip: for-profit parent SHOULD show the step in nav
    await page.goto(`${BASE_URL}/view/browser_test_parent/99902`);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);

    const navForProfit = page.locator('app-workflow-nav');
    assert('Workflow nav appears for for-profit parent', await navForProfit.count() > 0);

    if (await navForProfit.count() > 0) {
      const navText = await navForProfit.textContent();
      const hasDetailsStep = navText.includes('Additional Details');
      assert('Step visible for for-profit parent', hasDetailsStep,
        `Nav text: ${navText?.substring(0, 200)}`);
    }

    // === TEST 2: Review section submit button works ===
    console.log('\n  --- Review Submit Test ---');

    // 99902 has all steps complete, so review section should show
    const reviewSection = page.locator('app-workflow-review-section');
    assert('Review section visible on complete workflow', await reviewSection.count() > 0);

    if (await reviewSection.count() > 0) {
      const submitBtn = reviewSection.locator('button:has-text("Submit")');
      assert('Submit button visible in review section', await submitBtn.count() > 0);
      ...
      if (await submitBtn.count() > 0) {
        ...
        // After submit, the record should have submitted_at set
        // Verify by checking the page no longer shows the review section
        // (DetailPage sets showReviewSection=false when submitted)
        const reviewAfter = page.locator('app-workflow-review-section');
        assert('Review section hidden after submit', await reviewAfter.count() === 0);
      }
    }
  ...
  console.log('');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('  Workflow Conditions + Submit Test Results');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
